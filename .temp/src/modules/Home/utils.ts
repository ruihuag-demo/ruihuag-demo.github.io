...
// 判断路径出现id的情况[:id]
// /serviceInfo/function/:id    ----path
// /serviceInfo/function/123    ----url
// /serviceInfo/function/:id/subFunction/:cid
// /serviceInfo/function/123/subFunction/456
let link: string = '';

const equalPath: any = (url: string, path: string): boolean => {    //通过比较URL与path，得到与URL对应的path，并做了[:id]的转换
	link = '';
...
export const findURLtoPath =
	(list: iRouter[], url: string): iRouter[] => {
		...
		return result;    //最终得到URL改变后对应的面包屑
	}