...
import { Scrollbars } from 'react-custom-scrollbars'  //antd Table滚动条美化插件
    ...
    <div className='home-content-main-header'>
      <Breadcrumb>
        <Breadcrumb.Item>
          <Link to='/'>
            {' '}
            <FormattedMessage id={'menu.name._menu_home'} />
          </Link>
        </Breadcrumb.Item>
  ...
  const buildRouteTree: any = (list: iRouter[]): any => {
    return (
      ...
      list.map((item: iRouter): any => {
        ...
        return <Route exact={exact} key={key} path={path} component={module} />
      })
    )
  }
  return (
    <div className='home-content-main'>
      ...
      <Scrollbars className='home-content'>
        <Switch>{buildRouteTree(routers)}</Switch>
        <div className='home-content-space' />    {/*内容底部留白部分*/}
      </Scrollbars>
    </div>
  )
}
