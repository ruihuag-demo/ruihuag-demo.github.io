const Validates: (intl: any) => any = (intl: any): any => {
  return {
    required: {
      required: true,
      pattern: /^(?!\s*$).+/,
    ...
    cannotContain: (characters: string): any => {
      ...
      return {
        ...
        message: intl.formatMessage(
          { id: 'global.validate.cannotContain' },
    ...
    max: (num: number): any => {
      return {
        max: num,
        message: intl.formatMessage({ id: 'global.validate.max', value: num }, { value: num })
      } // 最大长度，传入一个number
    },
    min: (num: number): any => {
      return {
        min: num,
        message: intl.formatMessage({ id: 'global.validate.min', value: num }, { value: num })
      } // 最小长度，传入一个number
    },
